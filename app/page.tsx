...
export default function Home() {
  ...
  return (
    <main id="top" className="min-h-screen bg-neutral-50 text-neutral-800 font-sans scroll-smooth relative overflow-x-hidden">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-neutral-200 max-w-md w-full p-6 sm:p-8 relative text-neutral-800">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-neutral-500 mt-1">Provide authorization clearance below.</p>
            </div>
            ...
            {/* INLINE HINT CREDENTIALS PANEL */}
            <div className="mt-5 pt-4 border-t border-neutral-100 bg-neutral-50 p-3.5 rounded-xl border border-neutral-200 text-[11px] leading-relaxed text-neutral-600">
              <span className="font-bold block text-neutral-800 uppercase tracking-wide mb-1">Simulated Role Registry Master Keys:</span>
              <p>• Super Admin: <code className="bg-neutral-200 px-1 py-0.5 rounded font-mono font-bold select-all">superadmin</code> / password: <code className="bg-neutral-200 px-1 py-0.5 rounded font-mono font-bold select-all">super2026</code></p>
              <p>• Officer Layer: <code className="bg-neutral-200 px-1 py-0.5 rounded font-mono font-bold select-all">officer1</code> / password: <code className="bg-neutral-200 px-1 py-0.5 rounded font-mono font-bold select-all">officer2026</code></p>
              <p>• Member Layer: <code className="bg-neutral-200 px-1 py-0.5 rounded font-mono font-bold select-all">member1</code> / password: <code className="bg-neutral-200 px-1 py-0.5 rounded font-mono font-bold select-all">rotary3770</code></p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid lg:grid-cols-3 gap-8 items-start">
              <div className="bg-neutral-800 border border-neutral-700 p-4 sm:p-6 rounded-xl lg:col-span-2 space-y-6">
                      ...
                      {stageUsers.map(u => (
                        <tr key={u.id} className="hover:bg-neutral-700/50 transition flex flex-col md:table-row py-3 px-2 md:p-0 gap-1.5 md:gap-0 border-b border-neutral-700 md:border-none">
                          <td className="px-4 md:py-3 font-bold text-white block md:table-cell">
                            <span className="md:hidden text-neutral-500 font-normal block uppercase text-[9px] tracking-wider">Name</span>
                          ...
                          <td className="px-4 md:py-3 block md:table-cell">
                            <span className="md:hidden text-neutral-500 font-normal block uppercase text-[9px] tracking-wider mb-0.5">Role Authorization</span>
                            <span className={`inline-block px-2 py-0.5 rounded text-[9px] font-bold ${u.role === 'Super Admin' ? 'bg-neutral-900 text-blue-400 border border-neutral-700' : u.role === 'Officer' ? 'bg-neutral-900 text-neutral-300 border border-neutral-700' : 'bg-neutral-700 text-neutral-400'}`}>{u.role}</span>
                          </td>
                          <td className="px-4 md:py-3 text-neutral-400 block md:table-cell">
                            <span className="md:hidden text-neutral-500 font-normal block uppercase text-[9px] tracking-wider">Designation</span>
                            {u.position}
                          </td>
                          <td className="px-4 md:py-3 text-left md:text-right space-x-3 block md:table-cell mt-1 md:mt-0">
                            <span className="md:hidden text-neutral-500 font-normal block uppercase text-[9px] tracking-wider mb-1">Actions</span>
                            {(userSession.role === 'Super Admin' || userSession.id === u.id) ? (
                              <button suppressHydrationWarning onClick={() => setEditingUser(u)} className="text-blue-400 hover:underline font-bold bg-transparent border-none cursor-pointer text-xs">Edit</button>
                            ) : <span className="text-neutral-600 text-[10px]">Locked</span>}
                ...
                {editingUser && (
                  <form onSubmit={saveUserEditToStage} className="bg-neutral-900 p-4 rounded-lg border border-blue-500/30 grid sm:grid-cols-2 gap-4 text-xs text-neutral-800">
                    <div className="sm:col-span-2 flex justify-between items-center text-white border-b border-neutral-700 pb-1.5">
                      <span className="font-bold">Edit Staging User Properties</span>
                      <button suppressHydrationWarning type="button" onClick={() => setEditingUser(null)} className="text-neutral-400 hover:text-white bg-transparent border-none cursor-pointer">✕ Cancel</button>
                    </div>
                    <div>
                    ...
                    <button suppressHydrationWarning type="submit" className="sm:col-span-2 bg-blue-600 text-white font-bold py-2.5 rounded text-xs hover:bg-blue-700 transition border-none cursor-pointer">Save to Staging Cache</button>
                  </form>
                )}
                ...
                {userSession.role === 'Super Admin' ? (
                  ...
                  <p className="text-xs text-neutral-500 italic bg-neutral-900 p-3 rounded text-center border border-neutral-800">🔒 Roster additions are exclusively reserved for the Super Admin level.</p>
                )}
              </div>
              ...
              <div className="bg-neutral-800 border border-neutral-700 p-4 sm:p-6 rounded-xl space-y-6">
                ...
                {userSession.role === 'Super Admin' ? (
                  ...
                  <p className="text-xs text-neutral-400 italic bg-neutral-900 p-4 rounded border border-neutral-700 text-center">🔒 Content publishing configurations are locked under Super Admin clearances.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-10 lg:gap-12 items-center">
            ...
            <div className="bg-neutral-50 p-6 sm:p-8 rounded-xl border border-neutral-200 shadow-sm">
              ...
              <div className="border-l-4 border-blue-600 pl-4 space-y-4">
                ...
                <div>
                  <h4 className="font-bold text-neutral-900 text-sm">Community Partners</h4>
                  <p className="text-xs text-neutral-500 mt-0.5">We collaborate natively with local public schools, barangay health centers, and municipal bodies to pinpoint high-need civic operations.</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="officers" className="py-16 px-4 sm:px-6 bg-neutral-50 border-b border-neutral-200 scroll-mt-16">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <span className="text-blue-600 font-bold uppercase tracking-widest text-xs block">Club Leadership</span>
            <h2 className="text-3xl font-black text-neutral-900 mt-1 tracking-tight">Club Administration & Officers</h2>
            <p className="text-sm text-neutral-600 mt-2">Duly certified administrative registry of executives presiding over local operational developments.</p>
          </div>
      ...
      <section id="pillars" className="py-16 sm:py-20 px-4 sm:px-6 max-w-7xl mx-auto scroll-mt-16">
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
          <h2 className="text-2xl sm:text-3xl font-black text-neutral-900 tracking-tight">Areas of Service Focus</h2>
      ...
      <section id="portfolio" className="py-16 sm:py-20 bg-neutral-50 px-4 sm:px-6 border-b border-neutral-200 scroll-mt-16">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-black text-neutral-900 tracking-tight">Club Projects & News Feed</h2>
              <p className="text-xs sm:text-sm text-neutral-600 mt-1">Real-time content matrix showing field outcomes (Protected by staging deployment approvals).</p>
            </div>
            ...
            {filteredProdActivities.map((activity) => (
              <div key={activity.id} className="bg-white rounded-xl shadow-sm border border-neutral-200 p-5 sm:p-6 flex flex-col justify-between hover:shadow-md transition relative group">
                ...
                <div className="mt-6 pt-4 border-t border-neutral-100 flex justify-between items-end">
                  <div>
                    <span className="text-[10px] uppercase tracking-wider font-bold text-neutral-400 block mb-0.5">Recorded Impact / Date</span>
                    <p className="text-neutral-900 font-medium text-xs">{activity.detail}</p>
                  </div>
                  {userSession?.role === 'Super Admin' && (
      ...
      <section id="contact" className="py-16 sm:py-20 px-4 sm:px-6 max-w-4xl mx-auto scroll-mt-16">
        ...
        {activeForm === 'donate' && (
          <form name="donation-pledges" method="POST" data-netlify="true" className="space-y-4 bg-white p-6 sm:p-8 rounded-xl border border-neutral-200 shadow-sm text-xs sm:text-sm">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-neutral-500 uppercase mb-1">Donor Name / Organization</label>
            ...
            <div>
              <label className="block text-xs font-bold text-neutral-500 uppercase mb-1">Pledge or Support Description</label>
              <textarea name="pledge_details" rows={3} required className="w-full border border-neutral-300 rounded-lg px-4 py-2.5 text-xs focus:outline-none focus:border-blue-600" placeholder="Detail your pledge here..."></textarea>
            </div>
