...
const corporateSponsors = [
  ...
  { name: "Rotaract Club of Meycauayan Metro", logoImage: "/partner-rotaract-logo.png", fallbackText: "RCMM", url: "https://www.facebook.com/rotaractmeycauayanmetro/" },
...
export default function Home() {
  ...
  const allProcessedActivities = dashActivities.filter(item => {
  ...
  const getFilteredAndSortedVisionaries = () => {
    const subset = dashUsers.filter(user => {
  ...
  */
  ...
  return (
    <main id="top" className="min-h-screen bg-neutral-50 text-neutral-800 font-sans scroll-smooth relative overflow-x-hidden">
      ...
      <style jsx global>{`
        .custom-magazine-scrollbar::-webkit-scrollbar { width: 5px; }
        .custom-magazine-scrollbar::-webkit-scrollbar-track { background: #171717; border-radius: 999px; }
        .custom-magazine-scrollbar::-webkit-scrollbar-thumb { background: #d97706; border-radius: 999px; transition: background 0.2s ease-in-out; }
        .custom-magazine-scrollbar::-webkit-scrollbar-thumb:hover { background: #f59e0b; }
        .custom-magazine-scrollbar { scrollbar-width: thin; scrollbar-color: #d97706 #171717; }
        @keyframes infiniteMarquee { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
        .animate-marquee-stream { display: flex; width: max-content; animation: infiniteMarquee 28s linear infinite; }
        .animate-marquee-stream:hover { animation-play-state: paused; }
        .perspective-1000 { perspective: 1000px; }
        .transform-style-3d { transform-style: preserve-3d; }
        .backface-hidden { backface-visibility: hidden; }
        .rotate-y-180 { transform: rotateY(180deg); }
        @keyframes floatConfetti { 0% { transform: translateY(-20px) rotate(0deg); opacity: 1; } 100% { transform: translateY(100vh) rotate(720deg); opacity: 0; } }
        ...
        <>
                  ...
                  <div className="bg-neutral-950 text-white rounded-3xl p-6 sm:p-10 shadow-2xl flex flex-col justify-between min-h-[580px] relative border border-amber-500/20 overflow-hidden">
                    ...
                    <div className="mt-8 pt-4 border-t border-neutral-100 flex justify-between items-center relative z-10">
                      ...
                      <div className="flex gap-2.5">
                        <button {...hydration} onClick={handlePrevMessage} className="w-9 h-9 flex items-center justify-center rounded-full border border-neutral-800 bg-neutral-900 text-white hover:border-amber-500 hover:text-amber-500 transition-all duration-300 pointer font-black text-sm select-none">‹</button>
                        <button {...hydration} onClick={handleNextMessage} className="w-9 h-9 flex items-center justify-center rounded-full border border-neutral-800 bg-neutral-900 text-white hover:border-amber-500 hover:text-amber-500 transition-all duration-300 pointer font-black text-sm select-none">›</button>
                      </div>
                    </div>
                  </div>

                  <div className="bg-amber-50 text-black p-6 rounded-3xl shadow-lg border border-amber-600/20 text-left">
          ...
          >
            ...
            <div className="max-w-5xl mx-auto space-y-12 relative z-10 px-4 sm:px-6">
              ...
              <div className="grid sm:hidden max-w-xs mx-auto relative group">
          ...
          <section className="py-16 bg-white border-b border-neutral-200 overflow-hidden select-none">
            ...
            <div className="relative w-full flex items-center">
              ...
              <div className="animate-marquee-stream flex gap-12 items-center">
                {[...dashSponsors, ...dashSponsors, ...dashSponsors].map((sponsor, sIdx) => (
                  <a 
                    key={sIdx}
                    href={sponsor.url}
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-hidden animate-fadeIn">
              ...
              <div className="bg-white text-neutral-800 rounded-[32px] w-full max-w-6xl h-[85vh] shadow-2xl relative z-10 border border-neutral-200 flex flex-col animate-scaleUp overflow-hidden">
                <div className="bg-white border-b border-neutral-100 px-6 sm:px-10 py-6 flex justify-between items-center shrink-0">
                  ...
                  <button onClick={() => setSelectedActivity(null)} className="w-10 h-10 rounded-full bg-neutral-100 hover:bg-black text-neutral-500 hover:text-white flex items-center justify-center transition-all duration-200 font-mono text-xs font-bold border-none cursor-pointer outline-none shadow-sm">✕</button>
                </div>
                <div className="flex-1 p-6 sm:p-10 overflow-y-auto custom-magazine-scrollbar">
                  <div className="grid lg:grid-cols-12 gap-10 items-start">
                    <div className="lg:col-span-7 flex flex-col justify-between min-h-[460px] text-left">
                      ...
                      <div className="pt-4 border-t border-neutral-100 space-y-4 bg-white relative">
                          ...
                          <div className="relative inline-block text-left w-full sm:w-auto" ref={shareMenuRef}>
                            ...
                            {isShareMenuOpen && (
                              <div className="absolute left-0 bottom-full mb-2 w-full sm:w-48 bg-white border border-neutral-200 rounded-xl shadow-xl z-50 overflow-hidden animate-fadeIn py-1">
                                <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('https://rcmeycauayanmetro.org')}&quote=${encodeURIComponent(`Check out the latest from Rotary Club of Meycauayan Metro: ${selectedActivity.title}`)}`} target="_blank" rel="noopener noreferrer" onClick={() => setIsShareMenuOpen(false)} className="flex items-center px-4 py-2.5 text-xs font-bold text-neutral-700 hover:bg-neutral-50 hover:text-[#1877F2] tracking-wide uppercase transition no-underline">Facebook</a>
                                <a href="https://www.instagram.com" target="_blank" rel="noopener noreferrer" onClick={() => { setIsShareMenuOpen(false); alert('To share this ledger item to Instagram: Copy the project description details and upload to your stories or feed.'); }} className="flex items-center px-4 py-2.5 text-xs font-bold text-neutral-700 hover:bg-neutral-50 hover:text-purple-600 tracking-wide uppercase transition no-underline">Instagram</a>
                                <a href={`mailto:?subject=${encodeURIComponent(selectedActivity.title)}&body=${encodeURIComponent(`Check out the latest from Rotary Club of Meycauayan Metro: ${selectedActivity.title}\n\n${selectedActivity.description}`)}`} onClick={() => setIsShareMenuOpen(false)} className="flex items-center px-4 py-2.5 text-xs font-bold text-neutral-700 hover:bg-neutral-50 hover:text-black tracking-wide uppercase transition no-underline">Email App</a>
                              </div>
                            )}
                          </div>
                        </div>
                        <div className="bg-neutral-50 rounded-2xl p-4 border border-neutral-200/60 grid grid-cols-2 gap-4 shadow-inner">
                          ...
                          <div>
                            <span className="text-[9px] font-mono font-bold text-neutral-400 uppercase tracking-widest block mb-0.5">{selectedActivity.type === 'News' ? 'Log Timestamp Reference' : 'Ledger Detail Vector'}</span>
                    ...
                    <div className="lg:col-span-5 space-y-4 flex flex-col">
                      <span className="text-[10px] font-mono text-neutral-400 uppercase tracking-widest block border-b border-neutral-100 pb-1 text-left">Ref 🖼️ Media Portfolio Gallery</span>
                      <div onClick={() => setIsFullscreenLightbox(true)} className="w-full h-64 sm:h-80 bg-neutral-100 rounded-3xl overflow-hidden border border-neutral-200 relative group shadow-sm bg-cover bg-center transition-all duration-300 cursor-zoom-in" style={{ backgroundImage: `url('${modalActiveImage || '/rotary-logo.png'}')` }}>
                        <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-200 flex items-center justify-center">
                          <div className="bg-white/90 text-black text-xs font-black px-4 py-2.5 rounded-xl shadow-md uppercase tracking-wider scale-95 group-hover:scale-100 transition-transform duration-200">🔍 View Fullscreen</div>
                        </div>
                      </div>
                      <div className="grid grid-cols-4 gap-2.5">
                        {(selectedActivity?.galleryImages || []).slice(0, 4).map((imgSrc: string, imgIdx: number) => {
                          ...
                          return (
                            <button key={imgIdx} onClick={() => setModalActiveImage(imgSrc)} className={`h-16 sm:h-20 bg-neutral-50 rounded-xl overflow-hidden border bg-cover bg-center transition-all duration-200 shadow-sm outline-none cursor-pointer p-0 relative hover:scale-[1.04] ${modalActiveImage === imgSrc ? 'border-amber-500 scale-[0.97]' : 'border-neutral-200 hover:border-neutral-400'}`} style={{ backgroundImage: `url('${imgSrc}')` }}>
                              {isLastSlot && hasMoreFiles && (
                                <div className="absolute inset-0 bg-black/70 backdrop-blur-[1px] flex items-center justify-center text-white font-sans font-black text-sm sm:text-base tracking-wider select-none">+{remainingCount}</div>
                              )}
                            </button>
                          );
                        })}
          ...
          {isFullscreenLightbox && (
            <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-fadeIn">
              ...
              <button onClick={() => setIsFullscreenLightbox(false)} className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/10 hover:bg-white text-white hover:text-black flex items-center justify-center transition-all duration-200 font-mono text-sm font-bold border-none cursor-pointer outline-none shadow-lg z-10">✕</button>
